...
ngram <- function(file, N, comedian, title){
  df <- read_table(file, col_names = FALSE)
  
  df <- df %>% unnest_tokens(word, X1, token = "ngrams", n = N) %>% 
...
#Ali Wong
AW_Baby <- ngram("scraps//ali wong baby cobra.txt", 
                           "Ali Wong", "Baby Cobra")
Ali_Wong <- rbind(AW_Baby)%>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Amy Schumer
AS_Leather <- ngram("youtube//amy schumer the leather special.txt",
                              "Amy Schumer", "The Leather Special")
AS_Sex <- ngram("scraps//amy schumer mostly sex stuff.txt",
                          "Amy Schumer", "Mostly Sex Stuff")
AS_Live <- ngram("scraps//amy schumer live at the apollo.txt", 
                           "Amy Schumer", "Live at the Apollo")
Amy_Schumer <- rbind(AS_Leather, AS_Sex, AS_Live) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Aziz Ansari
AA_Delicious <- ngram("youtube//aziz ansari dangerously delicious.txt", 
                                "Aziz Ansari", "Dangerously Delicious")
AA_Buried <- ngram("youtube//aziz ansari buried alive.txt", 
                             "Aziz Ansari", "Buried Alive")
AA_Intimate <- ngram("scraps//aziz ansari intimate moments.txt",
                               "Aziz Ansari", "Intimate Moments for a Sensual Evening")
Aziz_Ansari <- rbind(AA_Buried, AA_Delicious, AA_Intimate) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Bill Burr
Burr_You <- ngram("youtube//bill burr you people are all the same.txt", 
                            "Bill Burr", "You People Are All The Same")
Burr_Why <- ngram("youtube//bill burr why do i do this.txt", 
                            "Bill Burr", "Why Do I Do This")
Burr_Let <- ngram("scraps//bill burr let it go.txt", "Bill Burr", "Let It Go")
Burr_Walk <- ngram("scraps//bill burr walk.txt", "Bill Burr", "Walk Your Way Out")
Bill_Burr <- rbind(Burr_Why, Burr_You, Burr_Let, Burr_Walk) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Bill Engvall
BE_Sign <- ngram("youtube//bill engvall heres your sign.txt",
                           "Bill Engvall", "Here's Your Sign")
Bill_Engvall <- rbind(BE_Sign)%>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Bill Hicks
BH_Rev <- ngram("scraps//bill hicks revelations.txt", "Bill Hicks", "Revelations")
BH_Relentless <- ngram("scraps//bill hicks relentless.txt", "Bill Hicks", "Relentless")
Bill_Hicks <- rbind(BH_Relentless, BH_Rev) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Bo Burnham
BB_Happy <- ngram('scraps//bo burnham make happy.txt', 'Bo Burnham', 'Make Happy')
BB_What <- ngram("youtube//bo burnham what.txt", "Bo Burnham", "What")
Bo_Burnham <- rbind(BB_Happy, BB_What) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Brian Posehn
BP_Crim <- ngram("youtube//brian posehn criminally posehn.txt", 
                           "Brian Posehn", "Criminally Posehn")
BP_JFL <- ngram("youtube//brian posehn just for laughs.txt", 
                          "Brian Posehn", "Just for Laughs")
Brian_Posehn <- rbind(BP_JFL, BP_Crim) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Chris Rock
CR_Pain <- ngram("youtube//chris rock bring the pain.txt", "Chris Rock", "Bring the Pain")
CR_BB <- ngram("youtube//chris rock bigger and blacker.txt", "Chris Rock", "Bigger and Blacker")
CR_Never <- ngram("youtube//chris rock never scared.txt", "Chris Rock", "Never Scared")
CR_Mess <- ngram("scraps//chris rock kill the messenger.txt", 
                           "Chris Rock", "Kill the Messenger")
Chris_Rock <- rbind(CR_BB, CR_Never, CR_Pain) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Daniel Tosh
DT_Serious <- ngram("youtube//daniel tosh completely serious.txt", 
                              "Daniel Tosh", "Completely Serious")
DT_People <- ngram("scraps//daniel tosh people pleaser.txt", 
                             "Daniel Tosh", "People Pleaser")
Daniel_Tosh <- rbind(DT_People, DT_Serious)%>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Dave Chappelle
DC_Killing <- ngram("youtube//dave chappelle killing them softly.txt",
                              "Dave Chappelle", "Killing Them Softly")
DC_Fwiw <- ngram("youtube//dave chappelle for what its worth.txt", 
                           "Dave Chappelle", "For What It's Worth")
DC_Texas <- ngram("scraps//dave chappelle texas.txt",
                            "Dave Chappelle", "Deep in the Heart of Texas")
DC_Spin <- ngram("scraps//dave chappelle age of spin.txt",
                           "Dave Chappelle", "The Age of Spin")
Dave_Chappelle <- rbind(DC_Fwiw, DC_Killing, DC_Texas, DC_Spin) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Demetri Martin
DM_If <- ngram("youtube//demetri martin if i.txt", "Demetri Martin", "If I")
DM_Live <- ngram("youtube//demetri martin live at the time.txt", 
                           "Demetri Martin", "Live, at the time")
DM_Benefit <- ngram("youtube//demetri martin benefit.txt", 
                              "Demetri Martin", "Benefit clip")
DM_Standup <- ngram("youtube//demetri martin standup comedian.txt", 
                              "Demetri Martin", "Standup Comedian")
Demetri_Martin <- rbind(DM_Benefit, DM_If, DM_Live, DM_Standup) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Donald Glover
DG_Weirdo <- ngram("youtube//donald glover weirdo.txt", "Donald Glover", "Weirdo")
Donald_Glover <- rbind(DG_Weirdo) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Eddie Murphy
EM_Raw <- ngram("youtube//eddie murphy raw.txt", "Eddie Murphy", "Raw")
EM_Del <- ngram("scraps//eddie murphy delirious.txt", "Eddie Murphy", "Delirious")
Eddie_Murphy <- rbind(EM_Raw, EM_Del) %>% group_by(word) %>% 
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Frankie Boyle
FB_Sodom <- ngram("scraps//frankie boyle the last days of sodom.txt", 
                            "Frankie Boyle", "The Last Days of Sodom")
FB_Live2 <- ngram("scraps//frankie boyle live 2.txt", "Frankie Boyle", "Live 2")
FB_Hurt <- ngram("scraps//frankie boyle hurt like youve never been loved.txt",
                           "Frankie Boyle", "Hurt Like You've Never Been Loved")
Frankie_Boyle <- rbind(FB_Hurt, FB_Live2, FB_Sodom) %>% group_by(word) %>% 
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Gabriel Iglesias
#GI_Haf <- ngram("scraps//gabriel iglesias hot and fluffy.txt", 
#                          "Gabriel Iglesias", "Hot and Fluffy")
#GI_Not <- ngram("scraps//gabriel iglesias not fat.txt", 
#                         "Gabriel Iglesias", "I'm Not Fat I'm Fluffy")


#George Carlin
GC_Again <- ngram("scraps//george carlin again.txt", "George Carlin", "Again")
GC_USC <- ngram("scraps//george carlin at usc.txt", "George Carlin", "Live at USC")
GC_Life <- ngram("scraps//george carlin life is worth losing.txt", 
                           "George Carlin", "Life is Worth Losing")
GC_Bad <- ngram("scraps//george carlin its bad for you.txt", "George Carlin",
                          "It's Bad For You")
GC_Diseased <- ngram("scraps//george carlin diseased.txt", "George Carlin",
                               "You Are All Diseased")
GC_Back <- ngram("scraps//george carlin back in town.txt", "George Carlin",
                           "Back in Town")
GC_Jam <- ngram("scraps//george carlin jamming.txt", 'George Carlin', "Jamming in New York")
George_Carlin <- rbind(GC_Again, GC_Life, GC_USC, GC_Bad, GC_Diseased, GC_Back, GC_Jam)%>% group_by(word) %>% 
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Hannibal Burress
HB_Animal <- ngram("youtube//hannibal burress animal furnace.txt", 
                             "Hannibal Burress", "Animal Furnace")
# HB_Comedy <- ngram("scraps//hannibal burress cc.txt", 
#                              "Hannibal Burress, Comedy Camisado")
Hannibal_Burress <- rbind(HB_Animal) %>% group_by(word) %>% 
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Iliza Schlesinger
IS_Hot <- ngram("youtube//iliza schlesinger freezing hot.txt",
                          "Iliza Schlesinger", "Freezing Hot")
IS_Kills <- ngram("youtube//iliza schlesinger confirmed kills.txt",
                            "Iliza Schlesinger", "Confirmed Kills")
Iliza_Sch <- rbind(IS_Hot, IS_Kills) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Jeff Foxworthy
JF_Totally <- ngram("youtube//jeff foxworthy totally committed.txt", 
                              "Jeff Foxworthy", "Totally Committed")
JF_Georgia <- ngram("youtube//jeff foxworthy in georgia.txt",
                              "Jeff Foxworthy", "Live in Georgia")
JF_Thinking <- ngram("youtube//jeff foxworthy weve been thinking.txt",
                               "Jeff Foxworthy", "We've Been Thinking")
Jeff_Foxworthy <- rbind(JF_Georgia, JF_Totally, JF_Thinking) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Jerry Seinfeld
JS_Telling <- ngram("youtube//jerry seinfeld telling you.txt", 
                              "Jerry Seinfeld", "I'm Telling You For The Last Time")
Jerry_Seinfeld <- rbind(JS_Telling)%>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Jim Gaffigan
JG_Universe <- ngram("youtube//jim gaffigan mr universe.txt", "Jim Gaffigan", "Mr Universe")
JG_Cinco <- ngram("scraps//jim gaffigan cinco.txt", "Jim Gaffigan", "Cinco")
Jim_Gaffigan <- rbind(JG_Cinco, JG_Universe)%>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Jim Jefferies
JJ_Dumb <- ngram("youtube//jim jefferies freedumb.txt", "Jim Jefferies", "Freedumb")
JJ_Contra <- ngram("youtube//jim jefferies contraband live.txt", "Jim Jefferies", "Contraband Live")
JJ_Alco <- ngram("youtube//jim jefferies alcoholocaust.txt", "Jim Jefferies", "Alcoholocaust")
JJ_God <- ngram("scraps//jim jefferies i swear to god.txt", "Jim Jefferies", "I Swear to God")
JJ_Bare <- ngram("scraps//jim jefferies bare.txt", "Jim Jefferies", "Bare")
Jim_Jefferies <- rbind(JJ_Dumb, JJ_Contra, JJ_Alco, JJ_God, JJ_Bare) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Jimmy Carr
JC_Funny <- ngram("youtube//jimmy carr funny business.txt", "Jimmy Carr", "Funny Business")
JC_Concert <- ngram("youtube//jimmy carr in concert.txt", "Jimmy Carr", "In Concert")
JC_Laugh <- ngram("youtube//jimmy carr making people laugh.txt", "Jimmy Carr", "Making People Laugh")
JC_Comedian <- ngram("youtube//jimmy carr comedian.txt", "Jimmy Carr", "Comedian")
Jimmy_Carr <- rbind(JC_Comedian, JC_Concert, JC_Funny, JC_Laugh) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#John Mulaney
JM_Comeback <- ngram("youtube//john mulaney comeback kid.txt", "John Mulaney", "The Comeback Kid")
JM_New <- ngram("youtube//john mulaney new in town.txt", "John Mulaney", "New in Town")
John_Mulaney <- rbind(JM_Comeback, JM_New) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Larry the Cable Guy
LC_Thinking <- ngram("youtube//larry the cable guy weve been thinking.txt",
                               "Larry the Cable Guy", "We've Been Thinking")
Larry_Cableguy <- rbind(LC_Thinking) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Lenny Bruce
LB_Berk <- ngram("scraps//lenny bruce berkeley.txt", "Lenny Bruce", "Berkeley")
LB_SA <- ngram("youtube//lenny bruce steve allen.txt", "Lenny Bruce", "The Steve Allen Show")
LB_Pal <- ngram("youtube//lenny bruce palladium.txt", "Lenny Bruce", "Palladium")
Lenny_Bruce <- rbind(LB_Berk, LB_Pal, LB_SA) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Louis CK
CK_Chewed <- ngram("youtube//louis ck chewed up.txt", "Louis CK", "Chewed Up")
CK_ONS <- ngram("youtube//louis ck one night stand.txt", "Louis CK", "One Night Stand")
CK_Beacon <- ngram("youtube//louis ck live at the beacon.txt", "Louis CK", "Live at the Beacon")
CK_Hilarious <- ngram("scraps//louis ck hilarious.txt", "Louis CK", "Hilarious")
CK_Shameless <- ngram("scraps//louis ck shameless.txt", "Louis CK", "Shameless")
CK_OMG <- ngram("scraps//louis ck omg.txt", "Louis CK", "Oh My God")
Louis_CK <- rbind(CK_Chewed, CK_ONS, CK_Beacon, CK_Hilarious, CK_Shameless, CK_OMG) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Maria Bamford
MB_Baby <- ngram("scraps//maria bamford old baby.txt", 
                           "Maria Bamford", "Old Baby")
MB_Special <- ngram("youtube//maria bamford specialx3.txt", "Maria Bamford",
                              "The Special Special Special")
Maria_Bamford <- rbind(MB_Baby, MB_Special) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Mitch Hedberg
# MH_Special <- ngram("scraps//mitch hedberg special.txt", "Mitch Hedberg", "Comedy Central Special")
# MH_Grill <- ngram("albums//mitch hedberg strategic grill locations.txt",
#                             "Mitch Hedberg", "Strategic Grill Locations")
MH_jokes <- ngram("mitch hedberg jokes.txt", "Mitch Hedberg", "Joke Compilation") %>% 
  filter(word != "listen")
Mitch_Hedberg <- rbind(MH_jokes) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Neal Brennan
NB_WBD <- ngram("scraps//neal brennan women and black dudes.txt",
                          "Neal Brennan", "Women and Black Dudes")
NB_3 <- ngram("scraps//neal brennan 3 mics.txt", "Neal Brennan", "3 Mics")
Neal_Brennan <- rbind(NB_3, NB_WBD) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Patton Oswalt
PO_Finest <- ngram("youtube//patton oswalt finest hour.txt", "Patton Oswalt", "Finest Hour")
PO_Talking <- ngram("youtube//patton oswalt talking for clapping.txt",
                              "Patton Oswalt", "Talking for Clapping")
PO_CCP <- ngram("netflix//patton oswalt comedy central presents.txt", 
                          "Patton Oswalt", "Comedy Central Presents")
PO_Ann <- ngram("scraps//patton oswalt annihilation.txt", "Patton Oswalt", "Annihilation")
Patton_Oswalt <- rbind(PO_CCP, PO_Finest, PO_Talking, PO_Ann) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Redd Foxx
RF_HBO <- ngram("youtube//redd foxx hbo special.txt", "Redd Foxx", "HBO Special")
RF_Chi <- ngram("youtube//redd foxx chicago.txt", "Redd Foxx", "Live in Chicago")
Redd_Foxx <- rbind(RF_HBO, RF_Chi) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Richard Pryor
RP_Here <- ngram("scraps//richard pryor here and now.txt",
                           "Richard Pryor", "Here and Now")
RP_Sunset <- ngram("scraps//richard pryor live on the sunset strip.txt",
                             "Richard Pryor", "Live on the Sunset Strip")
RP_Smoke <- ngram("scraps//richard pryor live and smokin.txt",
                            "Richard Pryor", "Live and Smokin")
RP_Live <- ngram("scraps//richard pryor live 79.txt",
                           "Richard Pryor", "Live In Concert")
Richard_Pryor <- rbind(RP_Here, RP_Smoke, RP_Sunset, RP_Live) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Ricky Gervais
RG_Out <- ngram("youtube//ricky gervais out of england.txt", 
                          "Ricky Gervais", "Out of England")
RG_O2 <- ngram("scraps//ricky gervais out of england 2.txt", 
                         "Ricky Gervais", "Out of England 2")
Ricky_Gervais <- rbind(RG_Out, RG_O2) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Ron White
RW_Unprof <- ngram("youtube//ron white a little unprofessional.txt", 
                             "Ron White", "A Little Unprofessional")
RW_Behavior <- ngram("youtube//ron white behavioral problems.txt", 
                               "Ron White", "Behavioral Problems")
RW_Tater <- ngram("youtube//ron white they call me tater salad.txt", 
                            "Ron White", "They Call Me Tater Salad")
Ron_White <- rbind(RW_Behavior, RW_Tater, RW_Unprof) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Steven Wright
SW_Special <- ngram("youtube//steven wright special.txt", "Steven Wright", "Special")
Steven_Wright <- rbind(SW_Special) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Steve Harvey
SH_Trip <- ngram("youtube//steve harvey dont trip.txt", "Steve Harvey", "Don't Trip")
SH_HBO <- ngram("youtube//steve harvey hbo special.txt", "Steve Harvey", "HBO Special")
Steve_Harvey <- rbind(SH_HBO, SH_Trip) %>% group_by(word) %>%
  summarize(n = sum(n), name = max(comedian)) %>% arrange(desc(n)) %>% mutate(rf = n / sum(n))

#Tom Segura
TS_Normal <- ngram("youtube//tom segura completely normal 2014.txt", 
                             "Tom Segura", "Completely Normal")
TS_Mostly <- ngram("scraps//tom segura mostly stories.txt", 
...
all_comparisons <- as.tibble(matrix(c(0), nrow = length(comedians_list), 
                                    ncol = length(comedians_list)))
